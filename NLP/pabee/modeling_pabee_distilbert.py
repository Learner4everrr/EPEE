...
"""PyTorch TinyBERT model with Patience-based Early Exit. """
...
from src.transformers.models.distilbert.modeling_distilbert import (
...
class TransformerWithPabee(Transformer):
...
class DistilBertModelWithPabeeV0(DistilBertModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder = TransformerWithPabee(config)
...
class DistilBertModelWithPabeeV1(DistilBertModel):
    """
...
class DistilBertModelWithPabeeV2(DistilBertModel):
    """
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids #, inputs_embeds=inputs_embeds #position_ids=position_ids,token_type_ids=token_type_ids,
        )
...
#         encoder_outputs = embedding_output
        ...
        if self.training or self.patience == -1:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if patient_counter == self.patience or cur_entropy < exiting_threshold/4:
                    break
...
class DistilBertForSequenceClassificationWithPabee(DistilBertPreTrainedModel):
    def __init__(self, config,):
        ...
        if config.ee_mechanism == "V0":
            self.bert = DistilBertModelWithPabeeV0(config)
        elif config.ee_mechanism == "V1":
            self.bert = DistilBertModelWithPabeeV1(config)
        elif config.ee_mechanism == "V2":
            self.bert = DistilBertModelWithPabeeV2(config)
        else:
            raise ValueError(
